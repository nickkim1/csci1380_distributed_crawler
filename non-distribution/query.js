...
function query(indexFile, args) {
  // Use path.join because __dirname is a absolute path to parent
  // dir of the script so it doesn't matter what dir the user is when they run query.js
  // this path will always resolve correctly
  const processedQueryStr = execSync(
      `echo ${args.join(' ')} | ${path.join(__dirname, './c/process.sh')}`,
      {
        encoding: 'utf-8',
      },
  ).slice(0, -1);
  const stemmedQueryStr = execSync(
      `echo ${processedQueryStr} | ${path.join(__dirname, './c/stem.js')}`,
      {
        encoding: 'utf-8',
      },
  ).slice(0, -1);
  const queryLen = stemmedQueryStr.length;
  // Get rid of the last char since echo adds an extra newline to the end
  ...
  fs.readFile(indexFile, 'utf8', (err, data) => {
    ...
    for (const line of globalIndexLines) {
      const terms = line.split('|')[0].trim();

      // Assumes no internal duplicates within the query str
      for (let i = 0; i < terms.length - queryLen + 1; i++) {
        if (terms.slice(i, i + queryLen) === stemmedQueryStr) {
          console.log(line);
          break;
        }
      }
