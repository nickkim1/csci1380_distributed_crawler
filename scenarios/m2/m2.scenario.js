...
test('characterize rpc', (done) => {
  const n = 1000;
  const remoteNode = {ip: '127.0.0.1', port: 9009};

  distribution.node.start((server) => {
    function setup(cb) {
      distribution.local.status.spawn(remoteNode, (e, v) => cb());
    };

    function benchRPC(cb) {
      let counter = 0;
      const addOne = () => {
        return ++counter;
      };
      ...
      const installRemote = {node: remoteNode, service: 'routes', method: 'put'};

      distribution.local.comm.send([rpcService, 'rpcPerf'], installRemote, (e, v) => {
        const rpcRemote = {node: remoteNode, service: 'rpcPerf', method: 'addOne'};

        const latencies = [];
        function measureLatency(count) {
          if (count === 0) {
            const avg = latencies.reduce((a, b) => a + b) / latencies.length;
            console.log(`RPC latency: ${avg.toFixed(4)} ms`);
            measureThroughput();
            return;
          }
          const start = performance.now();
          distribution.local.comm.send([], rpcRemote, () => {
          ...
          for (let i = 0; i < n; i++) {
            distribution.local.comm.send([], rpcRemote, () => {
              ...
              if (completed === n) {
                ...
                console.log(`RPC throughput: ${(n / totalTime).toFixed(4)} ops/ms`);
                return cb();
              }
            });
          }
    ...
    function cleanup(callback) {
      ...
      distribution.local.comm.send([],
          {node: remoteNode, service: 'status', method: 'stop'},
          callback);
    };
