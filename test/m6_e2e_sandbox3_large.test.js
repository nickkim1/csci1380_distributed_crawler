...
const gid = "sandbox3_e2e_test";
const indexGid = `index_${gid}`;
const workers = [
  { ip: "127.0.0.1", port: 7961 },
  { ip: "127.0.0.1", port: 7962 },
  { ip: "127.0.0.1", port: 7963 },
];
...
test("(integration) sandbox3 large crawl/index emits non-empty index and query results", (done) => {
  ...
  distribution[gid].crawler.exec(
    ...
    (crawlErr, crawlStats) => {
      ...
      try {
        ...
        const phase = crawlStats.crawlStats || {};
        expect(Number(phase.pagesFetched || 0)).toBeGreaterThan(0);
      } catch (assertErr) {
